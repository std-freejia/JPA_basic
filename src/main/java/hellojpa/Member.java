...
public class Member {
    ...
    /**
     * [무한루프 방지 팁]
     * 1) lombok에서 toString 만드는것 쓰지말기. (양방향 연관관계 걸려 있을 때 골치아픔)
     * 2) 컨트롤러에서 절대 엔티티를 바로 반환하지 말기. 엔티티는 꼭 DTO(값만 있는)로 변환하여 컨트롤러에서 반환하자.
     ...
     *
     * 3) '단방향 관계'만으로 충분히 설계 가능하다.
     * 역방향으로 '조회'가 필요할 때만 양방향 연관관계를 사용하자. 이 때 테이블설계를 바꿀 필요가 전혀 없고, 객체 관계이므로 코드만 조금 추가하면 된다.
     * 객체 관점에서 양방향 연관관계는 크게 이득되는건 없다.
     */
}
